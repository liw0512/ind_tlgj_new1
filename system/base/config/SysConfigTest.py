...
配置说明（自动补全）
====================
SysConfigTest：系统部署、数据库、实时周期、训练解释器和界面配置。
路径和数据库连接必须按部署机器修改；算法参数不应放在此文件。
修改后需要重启服务。数据库密码建议改为环境变量注入。
"""

config = {
    'base_path': "/home/lixingbest/ind_intell_v2/ind_optim_serv",  # 程序部署根目录；不同环境必须改为实际路径
    'filename': "/home/lixingbest/ind_intell_v2/logs",  # 日志输出目录
    'dbconnetion': "postgresql+psycopg2://root:tswcbyy5413LX@192.168.1.22:5432/ind_optim_serv",  # 数据库连接串；建议通过环境变量管理，不要提交真实生产密码
    "mod_args_path": "/home/lixingbest/ind_intell_v2/files/model",  # 模型参数文件目录
    "download_path": "/home/lixingbest/ind_intell_v2/ind_optim_serv/files",  # 下载、历史数据和模型文件的公共目录
