...
from .adapters.target_flow_view_model import normalize_target_flow_status
...
def _flow_event_title(flow: Mapping[str, Any]) -> str:
    current = _as_float(flow.get("current_flow"))
    peak = _as_float(flow.get("target_peak_flow"))
    final = _as_float(flow.get("target_final_flow"))
    shape = str(flow.get("flow_shape") or "").upper()
    if final is None:
        return "目标流量影子候选不可用"
    parts = []
    if current is not None:
        parts.append(f"当前 {current:.1f}")
    if shape in {"PULSE", "BOOST_STEP"} and peak is not None:
        parts.append(f"峰值 {peak:.1f}")
    parts.append(f"最终 {final:.1f}")
    return "目标流量影子：" + " → ".join(parts)
...
class HistoryDataService:
    ...
    def __init__(self, db_url: Optional[str] = None) -> None:
        ...
        # 连续缺失至少6个入库周期且超过3分钟才认为存在明确历史空窗，避免偶发漏点切碎曲线。
    ...
    def _extract_events(self, frame: pd.DataFrame) -> List[Dict[str, Any]]:
        ...
        previous_flow_signature: Optional[tuple[Any, ...]] = None
        previous_flow_state_signature: Optional[tuple[Any, ...]] = None
        last_flow_event_time: Optional[dt.datetime] = None
        ...
        for row in rows:
            ...
            if (
                ...
                previous_flow_signature = None
                previous_flow_state_signature = None
                last_flow_event_time = None
            ...
            flow_status = normalize_target_flow_status(
                _as_mapping(row.get("slurry_policy_target_supply_flow")),
                _as_mapping(row.get("slurry_policy_control_recommendation")),
                _as_mapping(
                    row.get("slurry_policy_target_flow_execution_preview")
                ),
            )
            has_flow_diagnostics = bool(
                flow_status.get("available")
                or flow_status.get("preview_status")
                or flow_status.get("prototype_id")
            )
            flow_signature = (
                flow_status.get("available"),
                flow_status.get("prototype_id"),
                flow_status.get("flow_shape"),
                round(flow_status["target_peak_flow"], 1)
                if flow_status.get("target_peak_flow") is not None
                else None,
                round(flow_status["target_final_flow"], 1)
                if flow_status.get("target_final_flow") is not None
                else None,
                flow_status.get("preview_status"),
                flow_status.get("tracking_state"),
                flow_status.get("effective_mode"),
            )
            ...
            flow_changed = flow_signature != previous_flow_signature
            state_changed = flow_state_signature != previous_flow_state_signature
            refresh_due = (
                last_flow_event_time is None
                or (timestamp - last_flow_event_time).total_seconds() >= 300.0
            )
            if has_flow_diagnostics and flow_changed and (state_changed or refresh_due):
                preview_ready = flow_status.get("preview_status") == "PREVIEW_READY"
                events.append({
                    "time": timestamp,
                    "type": "flow_candidate",
                    "title": _flow_event_title(flow_status),
                    "condition": condition,
                    "direction": flow_status.get("action_direction"),
                    "display_source": "供浆流量原型",
                    "display_status": (
                        "预演就绪"
                        if preview_ready
                        else "预演阻断"
                        if flow_status.get("preview_status") == "BLOCKED"
                        else "流量跟踪"
                    ),
                    "explanation": (
                        "已形成目标流量 DRY_RUN 路径，未向 DCS 写入指令"
                        if preview_ready
                        else "目标流量建议正在等待执行或安全条件确认"
                    ),
                    "mode": flow_status.get("effective_mode"),
                    "reason_codes": list(flow_status.get("reason_codes") or []),
                    "target_flow_status": flow_status,
                })
                last_flow_event_time = timestamp
                previous_flow_signature = flow_signature
            if has_flow_diagnostics:
                previous_flow_state_signature = flow_state_signature
