...
class SystemSettingsPage(QWidget):
    ...
    def update_runtime(self, data: Mapping[str, Any]) -> None:
        ...
        persistence = data.get("persistence_health")
        if not isinstance(persistence, Mapping):
            persistence = {}
        write_code = str(persistence.get("status_code") or "WAITING").upper()
        write_text = str(persistence.get("status_text") or "等待写入")
        if write_code == "ERROR":
            self.write_status.set_state("danger", write_text)
        elif write_code == "HEALTHY":
            self.write_status.set_state("normal", write_text)
        else:
            self.write_status.set_state("warning", write_text)
        self.write_status.setToolTip(
            str(persistence.get("last_error") or "写库状态来自实际异步写入结果")
        )
