...
class DataHealthCard(CardFrame):
    def __init__(self, parent=None):
        ...
        self.persistence = KeyValueRow("历史写库", "--")
        self.model_write_time = KeyValueRow("最近模型写入", "--")
    ...
    def update_data(self, data: Mapping[str, Any]) -> None:
        ...
        if not isinstance(persistence, Mapping):
            persistence = {}
        self.persistence.set_value(str(persistence.get("status_text") or "等待"))
        self.model_write_time.set_value(
            str(persistence.get("last_model_write_time") or "--")
        )
        jym = data.get("jym")
