...
def required_columns(plant: dict[str, Any]) -> list[str]:
    required = [
        time_column(plant),
        LOAD_COLUMN,
        INLET_SO2_COLUMN,
    ...
    required.extend(str(c) for c in (plant.get("supply_pump_state_columns", []) or []))
    return list(dict.fromkeys(required))


def selected_input_columns(plant: dict[str, Any]) -> list[str]:
    """Return every raw column used by the V1.8B training pipeline.

    Audit condition fields are included when available; missing optional audit
    columns are still added later as ``None`` exactly as in V1.7.
    """

    return list(dict.fromkeys([*required_columns(plant), *ALL_CONDITION_COLUMNS]))
...
def _read_csv_selected(
    ...
    *,
    ...
    if selected_columns is not None:
        usecols = lambda name: str(name).replace("\ufeff", "").strip() in selected_columns
    try:
        return pd.read_csv(
            path,
            encoding=encoding,
...
def load_input_data(
    ...
    selected = (
        set(selected_input_columns(plant))
    ...
    for file_index, path in enumerate(paths, start=1):
        ...
        if duplicates:
            raise InputDataError(f"CSV 标准化后存在重复字段: {duplicates}; 文件={path}")
        frame["__source_file"] = str(path)
        frames.append(frame)
        if progress:
            progress(0.05 + 0.45 * file_index / max(len(paths), 1), f"读取输入文件 {file_index}/{len(paths)}：{path.name}")
    df = pd.concat(frames, ignore_index=True, sort=False)
    if progress:
        progress(0.55, f"输入文件合并完成，共 {len(df)} 行")

    missing = [column for column in required_columns(plant) if column not in df.columns]
    if progress:
        progress(0.62, "校验必要字段")
    if missing and training["io"].get("strict_required_columns", True):
        raise InputDataError(
            "输入数据缺少第二模块必要字段: "
            f"{missing}。第一模块固定字段不需要配置映射，但必须存在于输出 CSV。"
        )

    # 审计字段当前第一模块会生成；若历史旧文件缺少个别审计字段，则补空值，
    # 不影响必要的 grid/region/state/valid 训练链路。
    ...
    if training["preprocessing"].get("coerce_numeric", True):
        ...
        numeric_list = list(dict.fromkeys(c for c in numeric_cols if c in df.columns))
        if numeric_list:
    ...
    skip_sort = bool(
        training.get("performance", {}).get("skip_sort_when_already_ordered", True)
    )
