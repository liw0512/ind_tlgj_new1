...
from .schema import INLET_SO2_COLUMN, LOAD_COLUMN, time_column
...
def calibrate_disturbance_thresholds(
    ...
    slopes_load: list[float] = []
    slopes_so2: list[float] = []
    load_col = LOAD_COLUMN
    inlet_col = INLET_SO2_COLUMN

    # 以非重叠窗口估计分布，避免高频行重复放大样本量。
    start = df[ts_col].min()
    end = df[ts_col].max()
    cursor = start
    while cursor < end:
        ...
        if len(block) >= 2:
            slopes_load.append(abs(robust_slope_per_minute(block[ts_col], block[load_col])))
            slopes_so2.append(abs(robust_slope_per_minute(block[ts_col], block[inlet_col])))
        cursor = block_end

    slow_q = float(cfg["auto_slow_quantile"])
    fast_q = float(cfg["auto_fast_quantile"])
    load_slow, load_fast = quantiles(pd.Series(slopes_load), (slow_q, fast_q))
    so2_slow, so2_fast = quantiles(pd.Series(slopes_so2), (slow_q, fast_q))

    cfg["load_slow_rate"] = max(
        float(cfg["minimum_load_slow_rate"]), 0.0 if np.isnan(load_slow) else load_slow
    )
    cfg["load_fast_rate"] = max(
        float(cfg["minimum_load_fast_rate"]),
        cfg["load_slow_rate"],
        0.0 if np.isnan(load_fast) else load_fast,
    )
    cfg["inlet_so2_slow_rate"] = max(
        float(cfg["minimum_inlet_so2_slow_rate"]),
        0.0 if np.isnan(so2_slow) else so2_slow,
    )
    cfg["inlet_so2_fast_rate"] = max(
        float(cfg["minimum_inlet_so2_fast_rate"]),
        cfg["inlet_so2_slow_rate"],
        0.0 if np.isnan(so2_fast) else so2_fast,
    )
...
def calibrate_action_magnitude_bins(
    ...
    if cfg.get("mode", "auto") != "auto":
        cfg["effective_family_bins"] = copy.deepcopy(cfg.get("family_fixed_bins", {}))
        cfg["effective_default_bins"] = copy.deepcopy(cfg["default_fixed_bins"])
        return cfg

    if episodes.empty or not {"episode_type", "valid", "action_magnitude_value"}.issubset(episodes.columns):
        action = pd.DataFrame(columns=["action_family", "action_magnitude_value"])
    else:
        action = episodes[(episodes["episode_type"] == "ACTION") & episodes["valid"]].copy()
    values = pd.to_numeric(action.get("action_magnitude_value", pd.Series(dtype=float)), errors="coerce").dropna()
    if values.empty:
        default = copy.deepcopy(cfg["default_fixed_bins"])
    else:
        default = {
            "small_max": float(values.quantile(float(cfg["small_quantile"]))),
            "medium_max": float(values.quantile(float(cfg["medium_quantile"]))),
        }
    default["small_max"] = max(float(cfg["micro_max"]), default["small_max"])
    default["medium_max"] = max(default["small_max"], default["medium_max"])
    ...
    for family, group in action.groupby("action_family", dropna=False):
        magnitudes = pd.to_numeric(group["action_magnitude_value"], errors="coerce").dropna()
        if len(magnitudes) < min_events:
            continue
        small = float(magnitudes.quantile(float(cfg["small_quantile"])))
        medium = float(magnitudes.quantile(float(cfg["medium_quantile"])))
        family_bins[str(family)] = {
            "small_max": max(float(cfg["micro_max"]), small),
            "medium_max": max(max(float(cfg["micro_max"]), small), medium),
        }
...
def calibrate_response_settings(
    ...
    if episodes.empty or "valid" not in episodes.columns:
        valid = pd.DataFrame(columns=["delta_outlet_so2", "post_outlet_so2_range"])
    else:
        valid = episodes[episodes["valid"]].copy()
    abs_delta = pd.to_numeric(
        valid.get("delta_outlet_so2", pd.Series(dtype=float)), errors="coerce"
    ).abs().dropna()
    if cfg.get("effect_strength_mode") == "auto" and not abs_delta.empty:
        q1 = float(abs_delta.quantile(float(cfg["effect_strength_small_quantile"])))
        q2 = float(abs_delta.quantile(float(cfg["effect_strength_medium_quantile"])))
    ...
    if cfg.get("stable_so2_range_max") is None:
        ranges = pd.to_numeric(
            valid.get("post_outlet_so2_range", pd.Series(dtype=float)), errors="coerce"
        ).dropna()
        cfg["effective_stable_so2_range_max"] = (
            float(ranges.quantile(0.75)) if not ranges.empty else 5.0
        )
    else:
        cfg["effective_stable_so2_range_max"] = float(cfg["stable_so2_range_max"])
    cfg["calibrated_from_data"] = True
    return cfg
...
def assign_response_labels(
    ...
    for row in result.itertuples(index=False):
        delta = float(row.delta_outlet_so2) if pd.notna(row.delta_outlet_so2) else np.nan
        if np.isnan(delta):
        ...
        stable_flags.append(
            bool(row.post_outlet_so2_range <= effective_response_cfg["effective_stable_so2_range_max"])
