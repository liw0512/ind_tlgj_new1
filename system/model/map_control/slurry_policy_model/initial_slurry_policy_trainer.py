"""供浆历史动作响应模型——初次离线训练入口。"""
from __future__ import annotations

import argparse
import json
import sys
from pathlib import Path

from slurry_policy_core import inspect_config, run_initial_training


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="从第一模块初次标注 CSV 建立第一版供浆历史动作响应版本"
    )
    parser.add_argument(
        "--input",
        nargs="+",
        help=(
            "输入 CSV、目录或通配符；未传时读取 "
            "PLANT_CONFIG.paths.default_initial_input"
        ),
    )
    parser.add_argument(
        "--output",
        help="输出根目录；未传时读取 PLANT_CONFIG.paths.output_root",
    )
    parser.add_argument(
        "--condition-snapshot",
        help=(
            "第一模块 condition_snapshot.json、v###目录或 snapshots根目录；"
            "未传时读取 PLANT_CONFIG.paths.condition_snapshots_dir 的最新版本"
        ),
    )
    parser.add_argument(
        "--config",
        help="统一配置文件路径或模块名；未传时使用同目录 slurry_policy_config.py",
    )
    parser.add_argument(
        "--allow-existing-output",
        action="store_true",
        help=(
            "允许输出目录已有版本时仍执行独立初次训练。通常不建议；"
            "已有模型继续学习应使用增量训练。"
        ),
    )
    parser.add_argument(
        "--no-progress",
        action="store_true",
        help="关闭终端训练进度条；默认按 TRAINING_CONFIG.progress.enabled 显示",
    )
    parser.add_argument(
        "--show-config",
        action="store_true",
        help="只打印完成默认值合并和校验后的有效配置，不执行训练",
    )
    return parser


def main(argv: list[str] | None = None) -> int:
    args = build_parser().parse_args(argv)
    try:
        if args.show_config:
            print(
                json.dumps(
                    inspect_config(args.config),
                    ensure_ascii=False,
                    indent=2,
                )
            )
            return 0

        snapshot = run_initial_training(
            input_paths=args.input,
            output_root=args.output,
            condition_snapshot=args.condition_snapshot,
            config_spec=args.config,
            allow_existing_output=args.allow_existing_output,
            progress_enabled=False if args.no_progress else None,
        )
        print(f"初次离线训练完成: {Path(snapshot).resolve()}")
        return 0
    except Exception as exc:
        print(f"初次离线训练失败: {exc}", file=sys.stderr)
        return 1


if __name__ == "__main__":
    raise SystemExit(main())
